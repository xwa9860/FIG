INPUT_MCNP_FILE = 'fuel_comp/inputs/Mark1.txt'
FLUX_CSV_PATH = 'fuel_comp/inputs/flux.csv'
OUTPUT_RAW_COMPS_FOLDER = 'fuel_comp/raw/'
OUTPUT_SQUARE_FLUX_AVE_COMP_FOLDER = 'fuel_comp/flux_sq_ave_serp/'
OUTPUT_FLUX_WALL_AVE_COMP_FOLDER = 'fuel_comp/flux_wall_ave_serp/'
OUTPUT_FLUX_ACT_AVE_COMP_FOLDER = 'fuel_comp/flux_act_ave_serp/'
RAW_COMPS_FOLDER = 'fuel_comp/raw_mcnp/'
# 160 raw mcnp material compositions from mark1.txt
OUTPUT_FUEL_FOLDER = 'FUEL_COMP'
